    ...
    if (Object.keys(data).includes("isFreeShipping")) {
      if (data.isFreeShipping !== "true" && data.isFreeShipping !== "false") {
        return res.status(400).send({
          status: false, message: "Not a valid value. isFreeShipping must be a boolean value",
        });
      }
    }
...
const getProductsByFilters = async (req, res) => {
  try {
    let data = req.query;
    let { size, name, priceGreaterThan, priceLessThan, priceSort, page } = data;
    ...
    let findData = await productModel.find(filterData).skip((page-1)*3).limit(3).sort({ price: priceSort });
...
const deleteByProductId = async (req, res) => {
  try {
    ...
    let deleteProduct = await productModel.findOneAndUpdate({ _id: productId, isDeleted: false }, {
      $set:
